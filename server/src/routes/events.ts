...
import type { LiveChannel } from "../lib/liveEvents.js"; //importing a TypeScript type only (import type removed at runtime so zero JS overhead)
...
const validChannels: LiveChannel[] = [ //what client is allowed to subscribe to
  "products",
  "orders",
  "batches",
  "history",
...
//After this, user is trusted
  ...
  const requestedChannels =
    typeof req.query.channels === "string" //Check if client passed
      ? req.query.channels
          .split(",")
          .map((channel) => channel.trim())
          .filter((channel): channel is LiveChannel => //Only allow valid channels
            validChannels.includes(channel as LiveChannel)
          )
