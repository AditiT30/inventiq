...
const sumOrderAmount = (products: unknown) => {
  if (!Array.isArray(products)) {
    return 0;
  }

  return (products as OrderProductSnapshot[]).reduce(
    (sum, item) => sum + (item.quantity ?? 0) * (item.unit_price ?? 0),
    0
  );
};

const sumBatchQuantity = (items: unknown) => {
  if (!Array.isArray(items)) {
    return 0;
  }

  return (items as BatchMaterialSnapshot[]).reduce((sum, item) => sum + (item.quantity ?? 0), 0);
};
...
const getBatchTone = (status: string) => {
  const normalized = status.toLowerCase();
...
const getManufacturingStage = (batch: { status: string; start_date: Date; end_date: Date | null }) => {
...
export const getDashboardSummaryHandler = async (
  ...
  try {
    ...
    const trendMap = new Map<string, { label: string; sales: number; purchases: number; sortKey: number }>();
    ...
    const salesByCustomer = new Map<string, { revenue: number; orders: number }>();
    for (const order of salesOrders) {
    ...
    for (const order of purchaseOrders) {
      ...
      purchasesBySupplier.set(
        supplierName,
        (purchasesBySupplier.get(supplierName) ?? 0) + sumOrderAmount(order.products)
      );
    }
    ...
    const salesContribution = buildContributionSplit(
    ...
    res.json({
      ...
      topCustomers,
      inventoryHealth: [
        { label: "Healthy", value: inventoryHealthRaw.healthy, tone: "emerald" as Tone },
        { label: "Low stock", value: inventoryHealthRaw.low, tone: "amber" as Tone },
        { label: "Out of stock", value: inventoryHealthRaw.out, tone: "rose" as Tone },
        { label: "Overstocked", value: inventoryHealthRaw.overstocked, tone: "sky" as Tone },
      ],
      contributionSplit: {
