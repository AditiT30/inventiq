...
//DEDUCT STOCK
export const deductStock = async (items, reason, tx = prisma) => {
    ...
    for (const item of items) {
        ...
        if (product.quantity < item.quantity) {
            throw new AppError(`Insufficient stock: ${item.product_code}`, 400);
        }
        //Update stock
...
// ADD STOCK
export const addStock = async (items, reason, tx = prisma) => {
    //Validate reason
    if (!reason) {
